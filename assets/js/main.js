/* ==========================================================
   ...
   Fokus: Desktop Scroll-Filme flüssiger, Mobile-Verhalten stabil lassen.
   ========================================================== */
...
function setupMobileVideoScrubbing(sectionSelector, videoSelector) {
  ...
  const update = () => {
    ...
    const rectTop = sectionTop - window.scrollY;
    const isRelevant = rectTop < viewportHeight * 1.1 && rectTop > -sectionHeight - viewportHeight * 0.1;
    ...
    const targetTime = clamp(progressValue * maxTime, 0.04, maxTime);
  ...
  video.addEventListener('loadedmetadata', () => { measure(); update(); }, { passive: true });
  video.addEventListener('canplay', update, { passive: true });
...
window.addEventListener('touchmove', () => requestFrame(2), { passive: true });
